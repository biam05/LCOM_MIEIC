...
int keyboard_manager()
{
  ...
  if(mini_game && !mg_enter && scancode[0] == ENTER_BREAK_CODE)
  {
    mg_enter = true;
    lost = false;
    score_counter = 0;
...
int mouse_manager()
{
  ...
  if(byte_no == 3)
  {
    ...
    else if(pack.lb == 1 && (pos_x >= SLEEP_MIN_X-20 && pos_x <= SLEEP_MAX_X-20) && (pos_y >= SLEEP_MIN_Y-10 && pos_y <= SLEEP_MAX_Y-10))
    {
      sleeping = true;
      eat = false;
      mini_game = false;
    ...
    else if(pack.lb == 1 && (pos_x >= FOOD_MIN_X-20 && pos_x <= FOOD_MAX_X-20) && (pos_y >= FOOD_MIN_Y-10 && pos_y <= FOOD_MAX_Y-10))
    {
      eat = true;
      sleeping = false;
      mini_game = false;
    ...
    else if(pack.lb == 1 && (pos_x >= PLAY_MIN_X-20 && pos_x <= PLAY_MAX_X-20) && (pos_y >= PLAY_MIN_Y-10 && pos_y <= PLAY_MAX_Y-10))
    {
      mini_game = true;
      eat = false;
      sleeping = false;
